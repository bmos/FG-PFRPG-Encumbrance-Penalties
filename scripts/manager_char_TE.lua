...
local function encumbrancePenalties(nodeChar)
    ...
    local total = DB.getValue(nodeChar,
                              CharEncumbranceManager.getEncumbranceField(), 0)
    ...
    DB.setValue(nodeChar, "encumbrance.encumbrancelevel", "number",
                nEncumbranceLevel)
    return nMaxStat, nCheckPenalty
end

local function isSpeedHalved(rActor)
    return EffectManager35EDS.hasEffectCondition(rActor, "Exhausted") or
               EffectManager35EDS.hasEffectCondition(rActor, "Entangled")
end

local function isSpeedNone(rActor)
    return ActorHealthManager.isDyingOrDead(rActor) or
               EffectManager35EDS.hasEffectCondition(rActor, "Grappled") or
               EffectManager35EDS.hasEffectCondition(rActor, "Paralyzed") or
               EffectManager35EDS.hasEffectCondition(rActor, "Petrified") or
...
local function hasSpecialAbility(nodeChar, sSpecAbil)
    ...
    for _, vNode in pairs(DB.getChildren(nodeChar, "specialabilitylist")) do
        local vLowerSpecAbilName = StringManager.trim(
                                       DB.getValue(vNode, "name", ""):lower());
        if vLowerSpecAbilName and
            string.find(vLowerSpecAbilName, sLowerSpecAbil, 0) then
            return true
        end
    end
end
...
local function getSpeedEffects(nodeChar)
    ...
    if DB.getValue(nodeChar, "hp.total", 0) ==
        DB.getValue(nodeChar, "hp.wounds", 0) then bSpeedHalved = true end

    --	Check if the character has fast movement ability
    local nSpeedAdj = 0
    if hasSpecialAbility(nodeChar, "Fast Movement") then
        local bEncumberedH = (DB.getValue(nodeChar,
                                          "encumbrance.encumbrancelevel", 0) >=
                                 2)
    ...
    nSpeedAdj = nSpeedAdj +
                    EffectManager35EDS.getEffectsBonus(rActor, "SPEED", true)
...
local function calcItemArmorClass_new(nodeChar)
    ...
    for _, vNode in pairs(DB.getChildren(nodeChar, "inventorylist")) do
        if DB.getValue(vNode, "carried", 0) == 2 then
            ...
            if bIsArmor then
                ...
                if bIsShield then
                    if bID then
                        nMainShieldTotal = nMainShieldTotal +
                                               DB.getValue(vNode, "ac", 0) +
                                               DB.getValue(vNode, "bonus", 0)
                    else
                        nMainShieldTotal = nMainShieldTotal +
                                               DB.getValue(vNode, "ac", 0)
                    end
                    ...
                    if bID then
                        nMainArmorTotal = nMainArmorTotal +
                                              DB.getValue(vNode, "ac", 0) +
                                              DB.getValue(vNode, "bonus", 0)
                    else
                        nMainArmorTotal = nMainArmorTotal +
                ...
                local nFighterLevel = DB.getValue(
                                          CharManager.getClassNode(nodeChar,
                                                                   "Fighter"),
                                          "level", 0)
                local bArmorTraining = (hasSpecialAbility(nodeChar,
                                                          "Armor Training") and
                                           nFighterLevel >= 3)
                local bArmorTrainingH = (bArmorTraining and nFighterLevel >= 7)
                local bAdvArmorTraining =
                    (hasSpecialAbility(nodeChar, "Advanced Armor Training"))
                ...
                if nMaxStatBonus > 0 then
                    ...
                    if nMaxStatBonus and nMainMaxStatBonus < 999 then
                        nMainMaxStatBonus =
                            math.min(nMainMaxStatBonus, nMaxStatBonus)
                    else
                        nMainMaxStatBonus = nMaxStatBonus
                    end
                else
                    for _, v in pairs(TEGlobals.tClumsyArmorTypes) do
                        if string.find(string.lower(
                                           DB.getValue(vNode, "name", 0)),
                                       string.lower(v)) then
                            nMainMaxStatBonus = 0
                            break
                        end
                ...
                if nCheckPenalty < 0 then
                    if not bIsShield and
                        CharManager.hasTrait(nodeChar, "Armor Expert") then
    ...
    if nEncMaxStatBonus then
        ...
        DB.setValue(nodeChar, "encumbrance.maxstatbonusfromenc", "number",
                    nEncMaxStatBonus)
    else
    ...
    if nEncCheckPenalty then
        ...
        DB.setValue(nodeChar, "encumbrance.checkpenaltyfromenc", "number",
                    nEncCheckPenalty)
    else
    ...
    DB.setValue(nodeChar, "encumbrance.armormaxstatbonus", "number",
                nMainMaxStatBonus)
    ...
    DB.setValue(nodeChar, "encumbrance.armorcheckpenalty", "number",
                nMainCheckPenalty)
    DB.setValue(nodeChar, "encumbrance.spellfailure", "number",
                nMainSpellFailure)
    ...
    local nSpeedAdjFromEffects, bSpeedHalved, bSpeedZero = getSpeedEffects(
                                                               nodeChar)
    ...
    if bApplySpeedPenalty == true then
        ...
        local nEncumbranceLevel = DB.getValue(nodeChar,
                                              "encumbrance.encumbrancelevel", 0)
    ...
    local nSpeedTotal = (nSpeedBase + nSpeedArmor +
...
function onInit()
    if Session.IsHost then
        DB.addHandler(DB.getPath("charsheet.*.hp"), "onChildUpdate",
                      onHealthChanged)
        DB.addHandler(DB.getPath("charsheet.*.wounds"), "onChildUpdate",
                      onHealthChanged)
        DB.addHandler(DB.getPath("charsheet.*.speed.base"), "onUpdate",
                      onSpeedChanged)

        DB.addHandler(DB.getPath(CombatManager.CT_COMBATANT_PATH ..
                                     '.effects.*.label'), 'onUpdate',
                      onEffectChanged)
        DB.addHandler(DB.getPath(CombatManager.CT_COMBATANT_PATH ..
                                     '.effects.*.isactive'), 'onUpdate',
                      onEffectChanged)
        DB.addHandler(DB.getPath(CombatManager.CT_COMBATANT_PATH .. '.effects'),
                      'onChildDeleted', onEffectRemoved)
    end
