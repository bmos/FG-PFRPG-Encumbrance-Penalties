...
function getEffectsByType(rActor, sEffectType, aFilter, rFilterActor,
                          bTargetedOnly)
    ...
    local bTargetSupport = StringManager.isWord(sEffectType,
                                                DataCommon.targetableeffectcomps);
        ...
        if (nActive ~= 0) then
            ...
            if not bTargeted or EffectManager.isEffectTarget(v, rFilterActor) then
                ...
                for kEffectComp, sEffectComp in ipairs(aEffectComps) do
                    local rEffectComp = EffectManager35E.parseEffectComp(
                                            sEffectComp);
                    -- Handle conditionals
                    if rEffectComp.type == "IF" then
                        if not EffectManager35E.checkConditional(rActor, v,
                                                                 rEffectComp.remainder) then
                            break
                        end
                    elseif rEffectComp.type == "IFT" then
                        if not rFilterActor then break end
                        if not EffectManager35E.checkConditional(rFilterActor,
                                                                 v,
                                                                 rEffectComp.remainder,
                                                                 rActor) then
                            break
                        end
                        bTargeted = true;
                        ...
                        for _, vPhrase in ipairs(rEffectComp.remainder) do
                            ...
                            local aPhraseOR = {};
                            repeat
                                local nStartOR, nEndOR = vPhrase:find(
                                                             "%s+or%s+",
                                                             nTempIndexOR);
                                if nStartOR then
                                    table.insert(aPhraseOR, vPhrase:sub(
                                                     nTempIndexOR,
                                                     nStartOR - nTempIndexOR));
                                    nTempIndexOR = nEndOR;
                                else
                                    table.insert(aPhraseOR,
                                                 vPhrase:sub(nTempIndexOR));
                                end
                            until nStartOR == nil;
                            ...
                            for _, vPhraseOR in ipairs(aPhraseOR) do
                                ...
                                repeat
                                    local nStartAND, nEndAND = vPhraseOR:find(
                                                                   "%s+and%s+",
                                                                   nTempIndexAND);
                                    if nStartAND then
                                        local sInsert = StringManager.trim(
                                                            vPhraseOR:sub(
                                                                nTempIndexAND,
                                                                nStartAND -
                                                                    nTempIndexAND));
                        ...
                        while aComponents[j] do
                            if StringManager.contains(DataCommon.dmgtypes,
                                                      aComponents[j]) or
                                StringManager.contains(DataCommon.bonustypes,
                                                       aComponents[j]) or
                                aComponents[j] == "all" then
                                -- Skip
                            elseif StringManager.contains(DataCommon.rangetypes,
                                                          aComponents[j]) then
                                ...
                                for _, v2 in pairs(aOtherFilter) do
                                    if type(v2) == "table" then
                                        ...
                                        for k3, v3 in pairs(v2) do
                                            if not StringManager.contains(
                                                aEffectOtherFilter, v3) then
...
function getEffectsBonusByType(rActor, aEffectType, bAddEmptyBonus, aFilter,
                               rFilterActor, bTargetedOnly)
    if not rActor or not aEffectType then return {}, 0; end

    -- MAKE BONUS TYPE INTO TABLE, IF NEEDED
    if type(aEffectType) ~= "table" then aEffectType = {aEffectType}; end
    ...
    for k, v in pairs(aEffectType) do
        ...
        local aEffectsByType = EffectManager35EDS.getEffectsByType(rActor, v,
                                                                   aFilter,
                                                                   rFilterActor,
                                                                   bTargetedOnly);
        ...
        for k2, v2 in pairs(aEffectsByType) do
            ...
            for _, v3 in pairs(v2.remainder) do
                if StringManager.contains(DataCommon.dmgtypes, v3) or
                    StringManager.contains(DataCommon.immunetypes, v3) or v3 ==
                    "all" then
                    dmg_type = v3;
                    break
            ...
            if dmg_type or not mod_type then
                ...
                local new_results = results[new_key] or
                                        {dice = {}, mod = 0, remainder = {}};
                ...
                local bStackable = StringManager.contains(
                                       DataCommon.stackablebonustypes, mod_type);
                if v2.mod >= 0 then
                    if bStackable then
                        bonuses[mod_type] = (bonuses[mod_type] or 0) + v2.mod;
                    else
                        bonuses[mod_type] = math.max(v2.mod,
                                                     bonuses[mod_type] or 0);
                    end
                elseif v2.mod < 0 then
                    if bStackable then
                        penalties[mod_type] =
                            (penalties[mod_type] or 0) + v2.mod;
                    else
                        penalties[mod_type] = math.min(v2.mod,
                                                       penalties[mod_type] or 0);
                    end
                end
...
function getEffectsBonus(rActor, aEffectType, bModOnly, aFilter, rFilterActor,
                         bTargetedOnly)
    ...
    for k, v in pairs(aEffectType) do
        ...
        local effbonusbytype, nEffectSubCount =
            EffectManager35EDS.getEffectsBonusByType(rActor, v, true, aFilter,
                                                     rFilterActor, bTargetedOnly);
        ...
        for k2, v2 in pairs(effbonusbytype) do
            ...
            if k2 == "" or StringManager.contains(DataCommon.dmgtypes, k2) then
                ...
                elseif v2.mod < 0 then
                    masterpenalties[k2] = math.min(v2.mod,
                                                   masterpenalties[k2] or 0);
                end
            end
        end
    ...
    for k, v in pairs(masterbonuses) do nTotalMod = nTotalMod + v; end
    for k, v in pairs(masterpenalties) do nTotalMod = nTotalMod + v; end

    if bModOnly then return nTotalMod, nEffectCount; end
...
function hasEffect(rActor, sEffect, rTarget, bTargetedOnly, bIgnoreEffectTargets)
    ...
    for _, v in pairs(DB.getChildren(ActorManager.getCTNode(rActor), "effects")) do
        ...
        if nActive ~= 0 then
            ...
            for kEffectComp, sEffectComp in ipairs(aEffectComps) do
                local rEffectComp =
                    EffectManager35E.parseEffectComp(sEffectComp);
                -- Check conditionals
                if rEffectComp.type == "IF" then
                    if not EffectManager35E.checkConditional(rActor, v,
                                                             rEffectComp.remainder) then
                ...
                elseif rEffectComp.type == "IFT" then
                    ...
                    if not EffectManager35E.checkConditional(rTarget, v,
                                                             rEffectComp.remainder,
                                                             rActor) then
                        break
                    end
...
function hasEffectCondition(rActor, sEffect)
    return EffectManager35EDS.hasEffect(rActor, sEffect, nil, false, true);
end
